...
function ParishForm({ church, allDioceses, onClose }) {
  ...
  function uploadTo(kind, idx) {
    ...
    inp.onchange = function () {
      ...
      window.uploadParishImage(file, church ? church.id : "new").then((url) => {
        if (kind === "hero") set("heroImage", url);
        else if (kind === "add") set("images", [...d.images, url]);
        else setImage(idx, url);
        setUploading("");
      }).catch((e) => { setUploading(""); setErr("Upload failed: " + (e.message || e)); });
    };
    inp.click();
  }
  ...
  function findOnline() {
    ...
    }).catch(() => { setFinding(false); setFindMsg("Couldn't reach the image service. Try again or paste a URL."); });
  }
  ...
  function save() {
    ...
    if (editing) {
      ...
      window.ParishStore.update(church.id, rec);    } else {
        ...
        <div className="modal-body">
          ...
          <div className="form-sec">
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <button className="add-row" onClick={() => uploadTo("add")} disabled={!!uploading}>
                {uploading ? <span className="spinner" style={{ width: 13, height: 13 }} /> : <window.I.upload style={{ width: 14, height: 14 }} />}
              ...
              <button className="add-row" onClick={addImage}><window.I.plus style={{ width: 14, height: 14 }} /> Add image link</button>
            </div>
          </div>
          {findMsg && <div className="find-msg"><window.I.sparkle style={{ width: 14, height: 14 }} /> {findMsg}</div>}
          <div className="form-grid">
            <FField label="Main / hero image" full hint="upload from your computer or paste a URL — shown on the parish page & cards">
              <div style={{ display: "flex", gap: 8 }}>
                <input value={d.heroImage} onChange={(e) => set("heroImage", e.target.value)} placeholder="https://…/photo.jpg" style={{ flex: 1 }} />
                <button className="add-row" onClick={() => uploadTo("hero")} disabled={!!uploading} style={{ flex: "none" }}>
          ...
          <div className="row-stack" style={{ marginTop: 10 }}>
            {d.images.map((url, i) => (
              <div className="dyn-row img" key={i}>
                <input value={url} onChange={(e) => setImage(i, e.target.value)} placeholder={"Gallery image " + (i + 1) + " URL"} />
                <button className="add-row" onClick={() => uploadTo("img", i)} disabled={!!uploading} style={{ flex: "none" }} title="Upload from computer">
                  {uploading === ("g" + i) ? <span className="spinner" style={{ width: 13, height: 13 }} /> : <window.I.upload style={{ width: 14, height: 14 }} />}
                </button>
                <button className="row-del" onClick={() => delImage(i)} aria-label="Remove"><window.I.trash style={{ width: 15, height: 15 }} /></button>
              </div>
            ))}
            {d.images.length === 0 && <div className="muted" style={{ fontSize: 13.5 }}>No gallery images yet — upload from your computer, find one online, or click any frame on the parish page.</div>}
          </div>
