/* compiled from map.jsx — do not edit directly; edit the .jsx source */
...
// escape any text before injecting it into popup HTML (defense in depth)
function esc(v) {
...
// only allow safe image URLs (http/https/relative) to become an <img src>
function safeUrl(u) {
  u = String(u || "").trim();
...
function popupHTML(c) {
  ...
  const img = safeUrl(c.heroImage || c.images[0]);
  const imgHTML = img ? `<img src="${esc(img)}" alt="" style="width:100%;height:100%;object-fit:cover;display:block" />` : `<div class="ph"><span>${esc(c.gallery[0])}</span></div>`;
  return `
    <div class="pop">
