...
function AdminView({ navigate, parishes }) {
  ...
  React.useEffect(() => {
    ...
    var rec = (all || []).find(function (c) { return c.id === pid; }) || window.ParishStore.get(pid);
  ...
  function doRefresh() { window.ParishStore.load().then(() => flash("Refreshed from the database.")); }
  function changePassword() {
    setPwErr("");
    if (pw1.length < 10) { setPwErr("Use at least 10 characters."); return; }
    if (pw1 !== pw2) { setPwErr("The two passwords don't match."); return; }
    setPwBusy(true);
    window.AdminAuth.changePassword(pw1).then((r) => {
      setPwBusy(false);
      if (r.ok) { setPwModal(false); setPw1(""); setPw2(""); flash("Password updated."); }
    ...
    <div className="admin">
      <div className="admin-hero">
        <div className="admin-hero-inner">
          <div>
            ...
            <p>Add, edit and import parish records. Changes are saved securely to your database and appear instantly across the directory, map and diocese views.</p>
          </div>
      ...
      <div className="admin-body">
        ...
        <div className="admin-toolbar">
          ...
          <button className="btn btn-ghost btn-sm" onClick={doRefresh}><window.I.reset /> Refresh</button>
          <button className="btn btn-ghost btn-sm" onClick={() => { setPwErr(""); setPw1(""); setPw2(""); setPwModal(true); }}><window.I.lock style={{ width: 14, height: 14 }} /> Change password</button>
        </div>
      ...
      {deleteTarget && (
        <div className="modal-overlay" onMouseDown={() => setDeleteTarget(null)}>
          <div className="modal modal-confirm" onMouseDown={(e) => e.stopPropagation()}>
            ...
            <p className="muted"><b>{deleteTarget.name}</b> will be permanently removed from the directory. This can’t be undone.</p>
      ...
      {pwModal && (
        <div className="modal-overlay" onMouseDown={() => setPwModal(false)}>
          <div className="modal modal-confirm" onMouseDown={(e) => e.stopPropagation()}>
            <div className="confirm-icon"><window.I.lock style={{ width: 22, height: 22 }} /></div>
            <h3>Change your password</h3>
            <p className="muted">Choose a new administrator password (at least 10 characters). You'll stay signed in.</p>
            <div style={{ textAlign: "left", marginTop: 6 }}>
              <label className="login-field"><span>New password</span>
                <input type="password" value={pw1} autoComplete="new-password" onChange={(e) => { setPw1(e.target.value); setPwErr(""); }} placeholder="••••••••••" /></label>
              <label className="login-field"><span>Confirm new password</span>
                <input type="password" value={pw2} autoComplete="new-password" onChange={(e) => { setPw2(e.target.value); setPwErr(""); }} placeholder="••••••••••" /></label>
              {pwErr && <div className="login-err"><window.I.warn style={{ width: 14, height: 14 }} /> {pwErr}</div>}
            </div>
            <div className="confirm-actions">
              <button className="btn btn-ghost" onClick={() => setPwModal(false)}>Cancel</button>
              <button className="btn btn-primary" onClick={changePassword} disabled={pwBusy}>
                {pwBusy ? <span className="spinner" /> : <window.I.check style={{ width: 16, height: 16 }} />} Update password
              </button>
            </div>
          </div>
        </div>
